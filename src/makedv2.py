...
if __name__ == "__main__":
    ...
    n_e = pd.DataFrame({"Hang_Voice":ser}, dtype=str)
    print("\n\n")
    ...
    '''
    print(string)
    n_index = get_search(df['str_column'], "hang|voice")
    
    n_e.loc[n_index,"Hang_Voice"] = "HangVoice"
    n_e.loc[~n_index, "New_Lead"] = "NotHangVoice"
    n_e.replace(to_replace = " ", value = np.nan, inplace = True)
    vcne =  n_e["Hang_Voice"].value_counts(dropna=False)
    print("THIS IS THE COUNTS OF 'HangVoice':")
    print(vcne, " with a length of ", len(n_e["Hang_Voice"]))
    print("THIS IS THE SUM OF THE VALUE COUNTS: ")
    print(vcne.sum())
    print("\n\n\n\n\n")
    df["Hang_Voice"]=n_e["Hang_Voice"]
    ...
    print(df.info())
    if save:
        df.to_csv("../data/cleaned_1.csv", index=False, compression="gzip")
        df.to_excel("../data/cleaned_1.xlsx", sheet_name="cleaned_1")